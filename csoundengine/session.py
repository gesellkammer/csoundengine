"""
This module provides a high-level interface to control un underlying
csound process. The core is a Session, which is associated with an Engine
...
generate code to read from the table and free the table after
...
.. code-block:: csound
    ...
    if i__paramslen < {maxidx} then
        initerror sprintf("params table too small (size: %d, needed: {maxidx})",
        i__paramslen)
    endif
    kamp tab 0, i_params
...
class Session:
    ...
    """
    A Session controls an engine. It has an exclusive associated
    Engine. An Engine can be thought of as a low-level
    interface to managing a csound instance, whereas a Session
    allows a higher-level control

    .. note:: 
    
        The user **does not** create an instance of this class directly.
        It is returned by either :meth:`Engine.session` or :func:`getSession`
    ...
    def __init__(self, name: str) -> None:
        ...
        """
        assert name not in self.activeSessions
...
def getSession(name="default") -> Opt[Session]:
    ...
    session = Session.activeSessions.get(name)
    if session:
