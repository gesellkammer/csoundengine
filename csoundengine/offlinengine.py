...
class OfflineEngine(_EngineBase):
    ...
    def perform(self, endtime=0., extratime=0.) -> None:
        ...
        Args:
            ...
            extratime: time added to the current end marker
        ...
        ~~~~~~~
            ...
            # for validation: the output file is only fully formed after the engine is stopped
            renderjob = e.stop()
    ...
    def cancel(self, remove=True) -> None:
        ...
        self.csound.stop()
        self.csound.cleanup()
        if remove and s.path.exists(self.outfile):
            logger.debug(f"Removing outfile '{self.outfile}'")
            os.remove(self.outfile)

    def stop(self, render=True) -> RenderJob | None:
        """
